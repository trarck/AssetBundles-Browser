...
namespace AssetBundleBuilder.Model
{
    public class AutoImport
    {
        ...
        protected Node GetNode(string assetName)
        {
            Node node = null;
        ...
        protected void ReplaceNode(Node from,Node to)
        {
        ...
        protected bool ExistsNode(string assetName)
        {
            return m_AssetsMap.ContainsKey(assetName);
        }

        protected Node MergeNode(Node to,Node from)
        {
            //合并资源
        ...
        {
            string[] deps = AssetDatabase.GetDependencies(node.mainAsset, false);
        ...
        protected BundleInfo CreateBundleInfo(Node node)
        {
            string mainAsset = node.mainAsset;

            BundleFolderInfo parent = null;
            if ((m_Format & Setting.Format.WithFolder) == Setting.Format.WithFolder)
            {
            ...
            string bundleName = Setting.CreateBundleName(mainAsset,m_Format);
        ...
        public void Import()
        {
            string assetsDataFile = GetAssetsDataFile();
            LoadAssetsData(assetsDataFile);
        ...
        public static void Test()
        {
            ...
            autoImport.m_Format = Setting.Format.WithFolder;

            autoImport.LoadFromFolder(Path.Combine(appPath, "Assets/ArtResources"), ".*\\.prefab");
            autoImport.Combine();
            autoImport.SaveAssetsData(Path.Combine(appPath, AssetBundleConstans.AssetsDataFile));
